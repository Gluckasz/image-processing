...
double ImageComparer::peakMeanSquareError(cv::Mat originalImage, const cv::Mat &newImage) {
    uchar max = 0;
...
double ImageComparer::signalToNoiseRatio(cv::Mat originalImage, cv::Mat newImage) {
    double squareSum = 0;
    double se = 0;
...
double ImageComparer::peakSignalToNoiseRatio(cv::Mat originalImage, cv::Mat newImage) {
    uchar max = 0;
    for (int x = 0; x < originalImage.cols; x++) {
        for (int y = 0; y < originalImage.rows; y++) {
            if (originalImage.at<uchar>(x, y) > max) {
...
double ImageComparer::maximumDifference(cv::Mat originalImage, cv::Mat newImage) {
