...
public:
    ...
    void apply(cv::Mat &newImage) const override {
        ...
        cv::split(noiseImage_, noiseImageChannels);
        cv::split(noNoiseImage_, noNoiseImageChannels);
        ...
        << "Peak mean square error before denoising: " << pmseBefore / static_cast<double>(newImageChannels.size()) << "\n"
        << "Peak mean square error after denoising: " << pmseAfter / static_cast<double>(newImageChannels.size()) << "\n"
        << "Signal to noise ratio before denoising: " << snrBefore / static_cast<double>(newImageChannels.size()) << "\n"
        << "Signal to noise ratio after denoising: " << snrAfter / static_cast<double>(newImageChannels.size()) << "\n"
        << "Peak signal to noise ratio before denoising: " << psnrBefore / static_cast<double>(newImageChannels.size()) << "\n"
        << "Peak signal to noise ratio before denoising: " << psnrAfter / static_cast<double>(newImageChannels.size()) << "\n"
        ...
        std::string path = OutputManager::constructPath("image", "comparison_stats", "txt");
